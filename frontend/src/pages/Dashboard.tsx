...
export function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [traders, evaluations] = await Promise.all([
          getAllTraders(),
          getAllEvaluations(),
        ]);

        const approved = evaluations.filter(e => e.decision === 'approved');
        const avgRoi = evaluations.length > 0
    ...
    <div className="py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <StatCard
          ...
          subtitle={stats.totalEvaluations > 0 ? `${((stats.approvedTraders / stats.totalEvaluations) * 100).toFixed(0)}% del total` : undefined}
          icon={
            <svg className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div>
                ...
                <p className="text-text-tertiary text-sm">Importar traders de Binance Copy Trading</p>
              </div>
            </Link>
            <Link
              to="/evaluations/new"
            ...
            >
              ...
              <div>
                ...
                <p className="text-text-tertiary text-sm">Evaluar un trader con criterios personalizados</p>
              </div>
            </Link>
          </div>
        </div>
          ...
          {recentEvaluations.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentEvaluations.map((evaluation) => (
                <div key={evaluation.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-bg-tertiary transition-colors">
                  <div>
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-medium ${
                    evaluation.decision === 'approved' ? 'bg-green-500/20 text-green-400' :
                    evaluation.decision === 'rejected' ? 'bg-red-500/20 text-red-400' :
